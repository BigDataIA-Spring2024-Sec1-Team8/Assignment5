...
def download_context_txt_from_s3(**context) -> str:
...
def download_from_s3(**context) -> str:
    ...
    s3 = boto3.client('s3', 
                      aws_access_key_id=aws_access_key_id, 
                      aws_secret_access_key=aws_secret_access_key)
    # s3_uri = 's3://cfainstitute-topic-details-processed/processed.csv'
    bucket_name, key,filename = extract_bucket_key_and_filename(s3_uri=s3_uri)
    print("buvk",bucket_name)
    print("buvkdd",key)
    print("buvddddddddk",filename)
